...
Open `gesture_review/index.html` in a browser to view the post-speech feedback prototype.
...
Later, the sample data in `gesture_review/app.js` can be replaced with real transcript, hand tracking, and PPG outputs from the Meta Aria pipeline.
...
```powershell
~/.venvs/speak_sync/bin/python aria_post_review/build_review_data.py --events data/processed/aria_events.json --transcript gesture_review/review_sample.json --output data/processed/review_data.json
```

Open `gesture_review/index.html`, click `Load SDK JSON`, and select `data/processed/review_data.json`.

To test the same conversion flow without glasses:

```powershell
~/.venvs/speak_sync/bin/python aria_post_review/build_review_data.py --events aria_post_review/sample_events.json --transcript gesture_review/review_sample.json --output data/processed/review_data.json
```
...
```bash
~/.venvs/speak_sync/bin/python aria_post_review/build_review_from_csv.py --csv path/to/features.csv --transcript gesture_review/review_sample.json --output data/processed/review_data.json
```
...
```bash
~/.venvs/speak_sync/bin/python aria_post_review/build_review_from_csv.py --csv aria_post_review/sample_features.csv --transcript gesture_review/review_sample.json --output data/processed/review_data.json
```
...
This also writes `gesture_review/review_data.generated.js`, so `gesture_review/index.html` opens with the latest generated CSV report automatically.
...
```bash
--transcript gesture_review/review_sample.json
```
...
- refreshes `gesture_review/review_data.generated.js` for the dashboard
